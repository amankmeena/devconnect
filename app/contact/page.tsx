        ...
        <>
            ...
            <section className="flex gap-8 justify-center items-start p-0 mx-auto max-w-[1200px] mb-12 flex-wrap contact-section">
                <div className="bg-(--panel-bg) rounded-(--radius) shadow-(--shadow) border-[1.5px] border-solid border-border p-7 pt-8 my-0 flex-1 min-w-[320px] max-w-[470px] panel">
                    ...
                    <form className="contact-form">
                        ...
                        <input className="w-full bg-(--input-bg) border border-(--input-border) text-white text-body rounded-[6px] px-3 py-[10px] mt-0.5 mb-0 focus:border-accent transition-colors duration-200 outline-none" type="text" id="name" placeholder="Enter your name" autoComplete="off" />

                        <label className="block text-body font-medium mb-1.5 mt-4.5 text-white" htmlFor="email">Email</label>
                        <input className="w-full bg-(--input-bg) border border-(--input-border) text-white text-body rounded-[6px] px-3 py-[10px] mt-0.5 mb-0 focus:border-accent transition-colors duration-200 outline-none" type="email" id="email" placeholder="your@email.com" autoComplete="off" />

                        <label className="block text-body font-medium mb-1.5 mt-4.5 text-white" htmlFor="subject">Subject</label>
                        <input className="w-full bg-(--input-bg) border border-(--input-border) text-white text-body rounded-[6px] px-3 py-[10px] mt-0.5 mb-0 focus:border-accent transition-colors duration-200 outline-none" type="text" id="subject" placeholder="What can we help you with?" />
                        <label className="block text-body font-medium mb-1.5 mt-4.5 text-white" htmlFor="message">Message</label>
                        <textarea className="w-full bg-(--input-bg) border border-(--input-border) text-white text-body rounded-[6px] px-3 py-[10px] mt-0.5 mb-0 min-h-[80px] resize-y focus:border-accent transition-colors duration-200 outline-none" id="message" placeholder="Tell us more about your inquiry or project..."></textarea>
            ...
            <section className="text-center mt-14 connect-section">
                ...
                <div className="flex gap-6.5 justify-center items-center text-body mb-0 connect-icons">
                    ...
                    <Link className="text-muted transition-colors duration-200 hover:text-accent" href="#" target="_blank" rel="noopener noreferrer"><span aria-label="LinkedIn"><FaLinkedin className="w-10 h-10" /></span></Link>
                    <Link className="text-muted transition-colors duration-200 hover:text-accent" href="#" target="_blank" rel="noopener noreferrer"><span aria-label="GitHub"><FaGithub className="w-10 h-10" /></span></Link>
                    <Link className="text-muted transition-colors duration-200 hover:text-accent" href="#" target="_blank" rel="noopener noreferrer"><span aria-label="Whatsapp"><FaWhatsapp className="w-10 h-10" /></span></Link>
                </div>
            </section>
        </>
    )
